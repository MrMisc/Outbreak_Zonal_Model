...
pub struct host{
    ...
    x:f64,
    y:f64,
    age:f64,  //3 additional parameters
    origin_x:u64,
    origin_y:u64,
    restrict:bool,  //Are the hosts free roaming or not?
    range_x:u64,  //"Internal" GRIDSIZE to simulate caged chickens in side the zone itself, not free roaming within facility
...
//Resolution
const STEP:usize = 20;  //Unit distance between chickens ->Could be used to make homogeneous zoning (Might not be very flexible a modelling decision)
...
impl host{
    fn infect(vector:&mut Vec<host>,loc_x:u64,loc_y:u64){
        if let Some(element) = vector.iter().find(|&x| x.origin_x-loc_x<STEP.try_into().unwrap() && x.origin_y-loc_y<STEP.try_into().unwrap() && x.motile==0){
            if let Some(first) = vector.first_mut(){
                *first=host{infected:true,motile:0,zone:first.zone,prob1:first.prob1,prob2:first.prob2,x:first.x,y:first.y,age:first.age,origin_x:first.origin_x,origin_y:first.origin_y,restrict:first.restrict,range_x:first.range_x,range_y:first.range_y};
            }
        }
    }
    ...
    fn new(zone:usize, std:f64,loc_x:u64, loc_y:u64,restriction:bool,range_x:u64,range_y:u64)->host{
        ...
        host{infected:false,motile:0,zone:zone,prob1:prob,prob2:std,x:loc_x as f64,y:loc_y as f64,age:normal(5.0*24.0,3.0*24.0,11.0*24.0), origin_x:loc_x,origin_y:loc_y,restrict:restriction,range_x:range_x,range_y:range_y}
    }
    fn new_inf(zone:usize, std:f64,loc_x:u64, loc_y:u64,restriction:bool,range_x:u64,range_y:u64)->host{
        let prob:f64 = LISTOFPROBABILITIES[zone.clone()];
        host{infected:true,motile:0,zone:zone,prob1:prob,prob2:std,x:loc_x as f64,y:loc_y as f64,age:normal(5.0*24.0,3.0*24.0,11.0*24.0), origin_x:loc_x,origin_y:loc_y,restrict:restriction,range_x:range_x,range_y:range_y}
    }
    fn deposit(self, consumable: bool)->host{ //Direct way to lay deposit from host. The function is 100% deterministic and layering a probability clause before this is typically expected
        ...
        if consumable{host{infected:inf,motile:1,zone:zone,prob1:prob1,prob2:prob2,x:x,y:y,age:0.0,origin_x:x as u64,origin_y:y as u64,restrict:restriction,range_x:range_x,range_y:range_y}}
        else{host{infected:inf,motile:2,zone:zone,prob1:prob1,prob2:prob2,x:x,y:y,age:0.0,origin_x:x as u64,origin_y:y as u64,restrict:restriction,range_x:range_x,range_y:range_y}}
    }
    ...
    fn shuffle(mut self)->host{
        if self.motile==0{
            ...
            if self.restrict{
                ...
                new_x = limits::min(limits::max(0.0,self.x+mult*normal(MEAN_MOVE,STD_MOVE,MAX_MOVE)),GRIDSIZE[0]);
                new_y = limits::min(limits::max(0.0,self.y+mult*normal(MEAN_MOVE,STD_MOVE,MAX_MOVE)),GRIDSIZE[1]);
            }
            
            host{infected:self.infected,motile:self.motile,zone:self.zone,prob1:self.prob1,prob2:self.prob2,x:new_x,y:new_y,age:self.age+1.0/HOUR_STEP,origin_x:self.origin_x,origin_y:self.origin_y,restrict:self.restrict,range_x:self.range_x,range_y:self.range_y}}
        else{
...
fn main(){
    ...
    for i in (0..GRIDSIZE[0] as u64).step_by(step){
        // println!("{}",i as f64)
        for j in (0..GRIDSIZE[1] as u64).step_by(step){
            chickens.push(host::new(1,0.2,i,j,true,STEP as u64,STEP as u64));
        }
    }
    ...
    host::infect(&mut chickens,400,400);
